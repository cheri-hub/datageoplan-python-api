...
# Cache do índice de busca (singleton)
_INDICE_BUSCA: Optional[Dict[str, Dict]] = None


def _criar_indice_busca() -> Dict[str, Dict]:
    """
...
def buscar_tema(tema_original: str) -> Optional[Dict]:
    """
    Busca informações sobre um tema no modelo CAR.
    
    Args:
        tema_original: Valor do campo "tema" do CAR
        
    Returns:
        Informações do tema ou None se não encontrado
    """
    indice = _criar_indice_busca()
    return indice.get(tema_original)


def get_cor_tema(tema_original: str) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    Returns:
        Tupla (cor_preenchimento, cor_contorno) ou (None, None)
    """
...
def listar_todos_temas() -> List[str]:
    """
...
def listar_grupos() -> List[Dict]:
    """
    Lista todos os grupos do modelo CAR.
    
    Returns:
        Lista de grupos com nome e ordem
    """
    grupos = []
    for classe_nome, classe_dados in MODELO_CAR.items():
        grupos.append({
    ...
    return sorted(grupos, key=lambda x: x["ordem"])


def listar_temas_por_grupo(grupo: str) -> List[Dict]:
    """
    Lista todos os temas de um grupo específico.
    
    Args:
        grupo: Nome da classe do grupo (ex: "Area_do_Imovel")
        
    Returns:
        Lista de temas do grupo
    """
    if grupo not in MODELO_CAR:
        return []
    
    return MODELO_CAR[grupo]["temas_possiveis"]


def contar_temas() -> Dict[str, int]:
    """
    Conta quantos temas existem no modelo.
    
    Returns:
        Contagem por classe e total
    """
    contagem = {"total": 0}
...
def obter_paleta_cores() -> Dict[str, Dict[str, str]]:
    """
    Retorna a paleta de cores de todos os temas.
    
    Returns:
        Dicionário com cores por tema
    """
    paleta = {}
    
    for classe_dados in MODELO_CAR.values():
        for tema_info in classe_dados["temas_possiveis"]:
            paleta[tema_info["arquivo_modelo"]] = {
                "tema_car": tema_info["tema_car"],
                "cor_preenchimento": tema_info["cor_preenchimento"],
                "cor_contorno": tema_info["cor_contorno"],
                "tipo": tema_info["tipo"]
            }
